...
export default function Intro() {
  ...
  const [jobData, setJobData] = useState([]); // Store jobs from CSV
  ...
  useEffect(() => {
    const fetchCSV = async () => {
      const response = await fetch('/path/to/your/jobs.csv'); // Update with the correct path to your CSV file
      ...
      Papa.parse(csvData, {
        ...
        complete: (results) => {
          setJobData(results.data); // Set job data from CSV
        },
  ...
  const handleSearch = (e) => {
    ...
    const filteredJobs = jobData.filter((job) => {
      const jobCategory = job.job_category; // Adjust the key based on your CSV structure
      ...
      <div className='w-full h-full flex items-center lg:justify-start py-24 justify-center flex-wrap'>
        <div className='lg:w-3/6 w-full sm:p-2 h-full my-2 flex items-center justify-center px-4 md:items-start md:justify-start md:p-20 flex-col'>
          ...
          <p className='md:text-lg sm:text-sm text-xs mb-20 text-gray-400'>
            Many Peoples are daily searching in this portal, 100 user added job portal!
          </p>
